...
// ДЗ
// Взять данные о рецептах по ссылке https://dummyjson.com/recipes
// Отрисовать карточки с рецептами - изображение, название, список ингредиентов, cookTimeMinutes, servings, cuisine, caloriesPerServing
// Выводить только те рецепты, в которых кол-во ккал на порцию меньше или равно 200
// Стилизовать контейнер с карточками рецептов - сделать грид из 2 колонок


const recipeContainer = document.getElementById('recipe_container');

recipes.recipes
    .filter(el => el.caloriesPerServing <= 200)
    .forEach(({image, name, ingredients, cookTimeMinutes, servings, cuisine, caloriesPerServing})=>{
    const recipeCard = document.createElement('div');
    const imageElem = document.createElement('img');
    const nameElem = document.createElement('h2');
    const ingredientsElem = document.createElement('ul');
    const ingredientsElemText = document.createElement('span');
    const cookTimeMinutesElem = document.createElement('p');
    const servingsElem = document.createElement('p');
    const cuisineElem = document.createElement('h3');
    
    const caloriesPerServingElem = document.createElement('p')

    imageElem.src = image;
    imageElem.alt = name;
    nameElem.innerText = name;
    cuisineElem.innerText = cuisine;
    cookTimeMinutesElem.innerText = `Время приготовления: ${cookTimeMinutes} минут`;
    servingsElem.innerText = `Колличество порций: ${servings}.`
    caloriesPerServingElem.innerText = `${caloriesPerServing} kkal.`;
    ingredientsElemText.innerText = 'Ингридиенты: '

    recipeCard.classList.add('recipe_card');
    recipeContainer.classList.add('recipe_container');
    imageElem.classList.add('image_elem');

    ingredients.forEach(el => {
        const ingredientsList = document.createElement('li');
        ingredientsList.innerText = el;
        ingredientsElem.append(ingredientsList);
    })

    recipeCard.append(nameElem, imageElem, cuisineElem, ingredientsElemText, ingredientsElem, cookTimeMinutesElem,servingsElem, caloriesPerServingElem);
